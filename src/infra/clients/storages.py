...
from types_aiobotocore_s3.type_defs import ObjectTypeDef, PutObjectOutputTypeDef

from src.infra.interfaces.storages import AbstractS3StorageClient


class S3StorageClient(AbstractS3StorageClient):
    async def upload_object(self, key: str, obj: BinaryIO) -> PutObjectOutputTypeDef:
    ...
    async def download_object(self, key: str) -> None:
        raise NotImplementedError()

    async def delete_object(self, key: str) -> None:
        raise NotImplementedError()

    async def object_post_url(self, key: str) -> dict[str, any]:
        async with self.get_client() as client:
            client: S3Client
