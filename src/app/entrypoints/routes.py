...
from src.products.domain.models.product import ProductUpdateModel, ProductModel
...
async def get_presign_urls(
    ...
    return await product_use_case.generate_presign_urls(filenames=filenames)
...
@router.patch(
    path="/{product_id}",
    response_model=ProductUpdateModel,
    status_code=status.HTTP_200_OK
)
async def update_product(
    ...
    product = await product_service.update_product(id_=product_id, product=ProductUpdateModel(**product.model_dump()))
    return ProductUpdateModel(**product.model_dump())
